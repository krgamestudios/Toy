...
int test_compiler_expressions(Toy_Bucket** bucketHandle) {
	//simple test to ensure the header looks right with an empty ast
	{
		...
		//run
		unsigned char* buffer = Toy_compileToBytecode(ast);
		...
		if ((ptr++)[0] != 28 || //total size
			...
			(ptr++)[0] != 0) //subs count
		{
			fprintf(stderr, TOY_CC_ERROR "ERROR: failed to produce the expected bytecode header, ast: PASS\n" TOY_CC_RESET);
		...
		{
			fprintf(stderr, TOY_CC_ERROR "ERROR: failed to produce the expected bytecode code, ast: PASS\n" TOY_CC_RESET);
	...
	//rerun the test with a more complex ast, derived from a snippet of source
	{
		...
		if ((ptr++)[0] != 28 || //total size
			...
			(ptr++)[0] != 0) //subs count
		{
			fprintf(stderr, TOY_CC_ERROR "ERROR: failed to produce the expected bytecode header, source: %s\n" TOY_CC_RESET, source);
	...
	//produce a string value
	{
		...
		if (
			...
			*(unsigned int*)(jumps + 0) != 0 || //the address relative to the start of the data section
			...
			false)
		{
			fprintf(stderr, TOY_CC_ERROR "ERROR: failed to produce the expected bytecode jumps, source: %s\n" TOY_CC_RESET, source);
		...
		if (
			...
			false)
		{
			fprintf(stderr, TOY_CC_ERROR "ERROR: failed to produce the expected bytecode data, source: %s\n" TOY_CC_RESET, source);
...
int test_compiler_string_reuse(Toy_Bucket** bucketHandle) {
	//test string literals
	{
		...
		if ((ptr++)[0] != 108 || //total size
			...
			false) //terminator
		{
			fprintf(stderr, TOY_CC_ERROR "ERROR: failed to reuse strings in bytecode header, source: %s\n" TOY_CC_RESET, source);
